function blurfinal_length2(cat,type)
...
for i=1:num_annot
    ...
    if(1==0)
        ...
        gh1=fspecial('gaussian',[round(h9/5) round(w9/5)],5.0);
        p=imfilter(final_obj1,gh1,'same');
        p50=p50+1;
        u=sprintf('tobefed1/%s/%s/%s/blurcontext2/%d.png',cat,namestore,type,p50);
        imwrite(p,u);
        %   imshow(p);
        %  pause;
        for j=1:length(partorder{1,1})
            ...
            p = svistest1(final_obj1,part_mask,2);
            p1=zeros(size(p));
            t1{j}=p;
            s1{j}=part_mask;
        ...
        for j=1:length(partorder{1,1})
            ...
            p = svistest1(obj_old,part_mask,2);
            t1{j}=p;
            s1{j}=part_mask;
            p1=p;
            [rrv1,ccv1]=find(part_mask);
