function imagelengthcontext(cat,type)
...
for i=1:num_annot-145
    ...
    p50=0;
    width1=right_orig-left_orig;
    height1=down_orig-up_orig;
    if(~isempty(v1.dim))
  ...
  %  final_obj=repmat(final_obj,[1,1,3]);
    ...
    if(h2>w2)
     ...
     z2=ones(size(mnew_final1(1).mask));
     part_mask=ff_2;
    p50=p50+1;
     for m=1:length(part)
      ...
      A=1.*z3;
        ...
        imshow(i1);
        pause;
    u=sprintf('tobefed1/%s/%s/%s/withcontext/%d.png',cat,namestore,type,p50);
   % imwrite(i1,u);
  % part_mask=zeros(size(mnew_final1(1).mask));
   part_mask=z3;
   ...
   % u=sprintf('tobefed1/%s/%s/%s/withcontext/%d.png',cat,namestore,type,p50);
    ...
    pause;
       ...
       obj_new=img_base(up_orig-5:down_orig+5,left_orig-5:right_orig+5,:);
       ff_new=ff(up_orig:down_orig,left_orig:right_orig);
       ff_new1=zeros(size(ff));
       ff_new1(up_orig:down_orig,left_orig:right_orig)=1;
       ff_new=ff_new1(up_orig-5:down_orig+5,left_orig-5:right_orig+5);
       part_mask=zeros(size(part(1).mask));
       z2=ones(size(part(1).mask));
       part_mask=ff_new;
        [h,w]=size(ff_new);
           p50=p50+1;
           for m=1:length(part)
