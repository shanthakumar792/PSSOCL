function imagesetwithoutcontext_temp(cat,type)
...
for i=1:num_annot
   ...
   % left_over=l2-part_mask;
    part_mask=zeros(size(part(1).mask));
     s4=sprintf('tobefed/%s/%s/%s/without_context/%d.png',cat,imname,type,1);
 %    imwrite(part_mask,s4);
    for j=1:length(partorder{1,1})
        ...
        a=uint8(a);
        i1=img.*a;
       s4=sprintf('tobefed/%s/%s/%s/without_context/%d.png',cat,imname,type,j+1);
        imshow(i1);
        pause;
  %     imwrite(i1,s4);
    ...
    k=img.*l1;
     imshow(k);
     pause;
    s4=sprintf('tobefed/%s/%s/%s/without_context/%d.png',cat,imname,type,j+2);
    %imwrite(k,s4);
end
end