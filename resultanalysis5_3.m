...
for i=1:6
    ...
    for j=3:num_cat+2
     cat=di(j).name;
     u1=sprintf('./vgg-19/%s/%s',cat,back);
     d1=dir(u1);
     num_scheme=length(d1);
     u3=sprintf('./vgg-19final1/%s/%s/%s',cat,context{1},'blurcontext2');
     load(u3);
     sk(j-2)=l4;
     ki=1;
      for k=3:num_scheme
          scheme=d1(k).name;
           u2=sprintf('./vgg-19final/%s/%s/%s.mat',cat,back,scheme(1:length(scheme)-4));
           load(u2);
           s1=h93;
           s2(q)=median(s1);
           s5(ki)=median(s1);
