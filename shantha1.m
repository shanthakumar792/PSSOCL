...
D = dir(anno_files);
all_annots = [];
fid=fopen('shantha1.txt','r');
cat=textscan(fid,'%s','\n');
fclose(fid);
k=zeros(1,length(cat{1,1}));
for i = 3:(length(D)-10000)
    ...
    for p=1:length(cat{1,1})
        t=0;
        for j=1:numel(anno.objects)
            if(strcmp(anno.objects(j).class,cat{1,1}{p,1}))
                t=1;
            end
        end
        if(t==1)
...
for q=1:length(cat{1,1})
    fprintf('number of images belonging to category [%s] = [%d]\n',cat{1,1}{q,1},k(q));
end
end
...
for oo = 1:numel(anno.objects)
    ...
    for pp = 1:numel(obj.parts)
        ...
        subplot(1,2,2);rectangle('Position',[part_minC part_minR bb_width_p bb_height_p],'EdgeColor','r');
