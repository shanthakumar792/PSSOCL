...
d=dir(s);
num_cat=length(d);
s6=sprintf('./partimportance2/%s',classifier);
for i=3:num_cat
    cat=d(i).name;
    mkdir(s6,cat);
    ...
    fclose(fid);
    parts=part{1,1};
    s5=sprintf('./partimportance2/%s/%s',classifier,cat);
    for j=3:num_things
        thing=d1(j).name;
        s2=sprintf('./partimportance/%s/%s/%s',classifier,cat,thing);
        fid=fopen(s2,'r');
        ...
        s4=sprintf('./partimportance2/%s/%s/%s.txt',classifier,cat,thing(1:(length(thing)-4)));
        fid=fopen(s4,'w');
        for k=1:length(p)
           times=p(k);
           toinsert=parts{k,1};
